...
int main()
{
    ...
    while (loan > 0){
        ...
        double pay = income + allowance - livingCost;
        cout << "So tien lai ngan hang tinh vao cuoi thang la: " << calculateLoan(loan) - loan << endl;
        loan = calculateLoan(loan) - pay;
        if (loan > 0) cout << "So tien du no cua sinh vien vao dau thang la: " << loan << endl;
        else {
                cout << "Sau " << i << " thang sinh vien hoan tra het no" << endl;
                cout << "Khoan thanh toan cuoi cung voi ngan hang la: " << loan + pay;
        }
        i++;
    }
    return 0;
